...
// DEBUG MAKRÓK
// ============================================================================
#define __SATDB_DEBUG
#if defined(__DEBUG) && defined(__SATDB_DEBUG)
#define SATDB_DEBUG(fmt, ...) DEBUG("SatelliteDb::" fmt __VA_OPT__(, ) __VA_ARGS__)
#else
#define SATDB_DEBUG(fmt, ...) // Üres makró, ha __DEBUG nincs definiálva
#endif
...
void SatelliteDb::debugSatDb(uint8_t num_sats_in_view) {
    ...
    SATDB_DEBUG("--------------------------------\n");
    SATDB_DEBUG("\n--- Visible Satellites ---\n");
    SATDB_DEBUG("Total in view (from GSV): %d\n", num_sats_in_view);
    SATDB_DEBUG("Total in DB: %d\n", countSats());
    SATDB_DEBUG("PRN | Elev | Azim | SNR | TTL(s)\n");
    SATDB_DEBUG("----|------|------|-----|-------\n");
