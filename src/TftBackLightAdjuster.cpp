...
// DEBUG MAKRÓK
// ============================================================================
// #define __TFT_BLADJUSTER_DEBUG
#if defined(__DEBUG) && defined(__TFT_BLADJUSTER_DEBUG)
#define TFT_BLADJUSTER_DEBUG(fmt, ...) DEBUG("TftBackLightAdjuster::" fmt __VA_OPT__(, ) __VA_ARGS__)
#else
#define TFT_BLADJUSTER_DEBUG(fmt, ...) // Üres makró, ha __DEBUG nincs definiálva
#endif
...
TftBackLightAdjuster::~TftBackLightAdjuster() { //
    config.unregisterCallback(configCallbackId);
}
    ...
    if (Utils::timeHasPassed(lastSensorCheckMsec, SENSOR_CHECK_MSEC)) {
        ...
        if (_tftAutoBrightnessActive) {
            ...
            if (lightSensorValue < SENSOR_VALUE_NIGHT) {
                ...
                // Map sensor range -> brightness: darker sensor -> lower brightness, brighter sensor -> higher brightness
                new_brightness = map(lightSensorValue, SENSOR_VALUE_NIGHT, SENSOR_VALUE_DAILY, NIGHTLY_BRIGHTNESS, DAILY_BRIGHTNESS);
            }

            // Debug: kiírjuk a szenzor és a cél / aktuális fényerő értékeket
            TFT_BLADJUSTER_DEBUG("LightSensor=%d target=%u current=%u\n", lightSensorValue, new_brightness, brightness);
