...
void ScreenGPSSetup::layoutComponents() {
    ...
    // Trafi Alarm gomb
    int row = 0;
    auto traffiAlarmButton = std::make_shared<UIButton>(                                            //
        10,                                                                                         //
    ...
    traffiAlarmButton->setUseMiniFont(false);
    addChild(traffiAlarmButton);
        ...
        [this, MIN_DISTANCE, MAX_DISTANCE, STEP_DISTANCE](const UIButton::ButtonEvent &event) {
            if (event.state == UIButton::EventButtonState::Clicked) {
                ...
                auto dialog = std::make_shared<ValueChangeDialog>(
                    this, "Trafi Alarm Distance", "100-1500 meters",                    //
    ...
    gpsTraffiAlarmDistanceBtn->setUseMiniFont(false);
    addChild(gpsTraffiAlarmDistanceBtn);

    row++;
    auto alarmSirenButton = std::make_shared<UIButton>(                                                  //
        12,                                                                                              //
        Rect(btnX, btnY + row * (btnH + btnGap), btnW, btnH),                                            //
        "Alarm Siren",                                                                                   //
        UIButton::ButtonType::Toggleable,                                                                //
    ...
    alarmSirenButton->setUseMiniFont(false);
    addChild(alarmSirenButton);
...
void ScreenGPSSetup::drawContent() {
    ...
    tft.drawString("GPS Settings", tft.width() / 2, 20);
    tft.setFreeFont();
}
