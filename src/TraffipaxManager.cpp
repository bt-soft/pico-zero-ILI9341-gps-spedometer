...
/**
 * @brief TraffipaxManager konstruktor
 */
TraffipaxManager::TraffipaxManager() {}

/**
 * @brief Ellenőrizzük, hogy létezik-e a CSV fájl
...
/**
 * @brief Betölti a Trafipax adatokat CSV fájlból (ISO-8859-2 kódólású a fájl!)
 * @param filename a fájl neve
 */
...
/**
 * @brief Visszaadja a Trafipaxok számát
 */
int TraffipaxManager::count() const { return traffipaxCount; }

/**
 * @brief Trafipax riasztás - csak közeledés esetén riaszt
...
/**
 * @brief Legközelebbi trafipax keresése távolsággal együtt
 * @param currentLat aktuális GPS szélesség
 * @param currentLon aktuális GPS hosszúság
...
/**
 * @brief Demo indítása - 5mp várakozás, majd közeledés/távolodás szimulálása
 */
void TraffipaxManager::startDemo() {
    ...
    if (traffipaxCount > 0) {
        ...
        DEBUG("Demo trafipax kiválasztva: %s - %s, lat: %2.6f, lon: %2.6f\n", //
...
/**
 * @brief Demo feldolgozása - szimulált GPS koordináták generálása
 */
...
/**
 * @brief Visszaadja, hogy aktív-e a demo
 * @return true ha a demo aktív, false ha nem
 */
bool TraffipaxManager::isDemoActive() const { return demo.isActive; }

/**
 * @brief Demo koordináták lekérése
 * @param lat kimeneti paraméter a szimulált szélességhez
 * @param lon kimeneti paraméter a szimulált hosszúsághoz
 * @return true ha van érvényes demo koordináta, false ha nincs
 */
