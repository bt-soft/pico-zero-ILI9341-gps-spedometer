...
void ScreenTFTSetup::layoutComponents() {
    ...
    int row = 0;
    auto autoBrightnessBtn = std::make_shared<UIButton>(                                              //
        10,                                                                                           //
    ...
    autoBrightnessBtn->setUseMiniFont(false);
    addChild(autoBrightnessBtn);
    ...
    manualBrightnessBtn->setUseMiniFont(false);
    manualBrightnessBtn->setEnabled(!config.data.tftAutoBrightnessActive);
...
void ScreenTFTSetup::drawContent() {
    ...
    tft.drawString("TFT Settings", tft.width() / 2, 20);
    tft.setFreeFont();
}
